...
public class CADU4 {
	...
	public CADU4(double[][] pData, int[] pRLabels, double pDc, double[] pMCost, double pTCost, double pLoose) {
		rLabel = pRLabels;
		data = pData;
		dc = pDc;
		mCost = pMCost;
	...
	public static void test() throws Exception {
		...
		for (String ds : strings) {
			...
			for (; dc < 0.601; dc += 0.05) {
				...
				CADU4 cadu = new CADU4(data, rLabels, dc, new double[] { 4, 9 }, tCost, loose);
	...
	public static void experimentDcAndLoose() throws Exception {
		...
		for (double loose = 0; loose <= 0.101; loose += 0.01) {
			for (double dc = 0.01; dc <= 0.15001; dc += 0.01) {
				double tCost = 1;
				CADU4 cadu = new CADU4(data, rLabels, dc, mCost, tCost, loose);
				cadu.computeMaxDistance();
				cadu.computeRho();
				cadu.computeRhoIndices();
				cadu.computeDelta();
