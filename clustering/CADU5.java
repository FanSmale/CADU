...
public class CADU5 {
	...
	public CADU5(double[][] pData, int[] pRLabels, double pDc, double[] pMCost, double pTCost, int pT, double pLoose) {
		rLabel = pRLabels;
		data = pData;
		dc = pDc;
		mCost = pMCost;
		...
		for (int i = 1; i < pPrioInd.length; i++) {
			...
			if (notPure[0] && notPure[1]) {
				...
				if (leaf[pPrioInd[i]]) {
					...
					int[] sortedBlocksub1 = new int[pPrioInd.length - 1];
					System.arraycopy(pPrioInd, 0, sortedBlocksub1, 0, i);
					System.arraycopy(pPrioInd, i+1, sortedBlocksub1, i, pPrioInd.length - i - 1);
	...
	public static void test() throws Exception {
		...
		CADU5 cadu = new CADU5(data, rLabels, dc, new double[] { 4, 9 }, tCost, T, loose);
